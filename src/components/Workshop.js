...
{workshop.map((item,workshoppp) => {
            return <div key={workshoppp}>   
            {item.status === 'approvedbyreviewer' && (     
                                   

              <Card  className="lkcardworkshop" hoverable style={{ marginBottom: 16 }} >
                    <div >                  
                     
                     <div className="lktexts"><h2 >Workshop Topic :- {item.workshopTopic}</h2></div> 
                      <h3>Conducted by {conductName}</h3>
                      <h4>Descrption: </h4><h6>{item.workshopDescription}</h6>
